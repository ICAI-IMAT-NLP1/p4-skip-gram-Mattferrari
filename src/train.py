...
def train_skipgram(model: SkipGramNeg,
    ...
    for epoch in range(epochs):
        for input_words, target_words in get_batches(words, batch_size, window_size):
            ...
            # TODO
            input_vectors = model.forward_input(torch.tensor(inputs))
            output_vectors = model.forward_output(torch.tensor(targets))
            noise_vectors = model.forward_noise(targets.shape[0], n_samples=3)
